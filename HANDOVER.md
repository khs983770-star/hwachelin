...
바로 이어서 한다면 **황금칸 랭킹 실제 데이터 연결**이 최우선.

이유:
- 로그인, 리뷰 작성/수정/삭제, 제보 접수까지 MVP 핵심 입력 흐름이 연결됨
- 지도와 상세 화면은 실제 리뷰 통계를 읽고 있음
- 이제 `황금칸`이 실제 리뷰 평점/개수 기반으로 움직여야 앱의 보상/탐색 흐름이 완성됨

다음 작업은 `app/(tabs)/gold.tsx`를 Supabase `reviews + toilets + places` 집계 데이터에 연결하고, 리뷰 수가 적은 장소가 과도하게 상위에 뜨지 않도록 최소 리뷰 수 또는 가중 점수 규칙을 넣는다.
